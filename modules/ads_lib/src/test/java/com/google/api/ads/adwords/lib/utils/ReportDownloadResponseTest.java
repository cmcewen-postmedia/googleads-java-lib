...
public class ReportDownloadResponseTest {
  ...
  public void testGetInputStream() throws IOException {
    ...
    InputStream inputStream = new ByteArrayInputStream(expectedContents.getBytes(Charset.forName("UTF-8")));
    RawReportDownloadResponse rawResponse = new RawReportDownloadResponse(HttpURLConnection.HTTP_OK,
        inputStream, Charset.forName("UTF-8"), "CSV");
    ReportDownloadResponse response = new ReportDownloadResponse(rawResponse);
    String actualContents = Streams.readAll(response.getInputStream(), Charset.forName("UTF-8"));
  ...
  public void testGetAsString_gzippedCSV() throws IOException {
    ...
    Streams.copy(new ByteArrayInputStream(expectedContents.getBytes(Charset.forName("UTF-8"))),
        new GZIPOutputStream(zippedBytesOut));

    RawReportDownloadResponse rawResponse = new RawReportDownloadResponse(HttpURLConnection.HTTP_OK,
        new ByteArrayInputStream(zippedBytesOut.toByteArray()), Charset.forName("UTF-8"),
        "GZIPPED_CSV");
    ReportDownloadResponse response = new ReportDownloadResponse(rawResponse);
  ...
  public void testSaveToFile() throws IOException {
    ...
    assertEquals("contents saved to file are incorrect", expectedContents,
        Streams.readAll(new FileInputStream(outputFile), Charset.forName("UTF-8")));
  }

  @Test
  public void testFailedResponse_fails() {
    RawReportDownloadResponse rawResponse = new RawReportDownloadResponse(
        HttpURLConnection.HTTP_BAD_REQUEST, new ByteArrayInputStream("failed".getBytes(Charset.forName("UTF-8"))),
