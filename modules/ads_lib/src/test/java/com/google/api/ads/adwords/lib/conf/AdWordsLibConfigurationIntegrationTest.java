...
/**
 * Verifies that we correctly wire in all properties files to our {@link AdWordsLibConfiguration}.
...
public class AdWordsLibConfigurationIntegrationTest {
  ...
  public void testPropertyFileIncludedInConfig() throws Exception {
    ...
    AdWordsLibConfiguration config =
        Guice.createInjector(new AdWordsModule()).getInstance(AdWordsLibConfiguration.class);
  ...
  private File createPropertiesFile() throws IOException {
    ...
    Writer fileWriter = new BufferedWriter(new OutputStreamWriter(
            new FileOutputStream("ads.properties"), "utf-8"));
